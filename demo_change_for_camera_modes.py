...
def parse_args():
    ...
    ####  added arguments for camera intrinsics and extrinsics / by TK
    parser.add_argument(
        "--camera_mode",
        type=str,
        default="default",
        choices=["default", "following", "stationary", "static"],
        help=("Camera behaviour:\n"
              "  default    = original demo.py behaviour from Human3R\n"
              "  following  = extrinsics vary; intrinsics fixed over time (example classic dashcam video)\n"
              "  stationary = no translation; rotation & intrinsics may vary (camera in professional racing event)\n"
              "  static     = no rotation, no translation, no zoom (fixed poses & intrinsics; surveillance camera etc.)")
    )
    parser.add_argument("--fx", type=float, default=None, help="Known focal length fx (pixels)")
    parser.add_argument("--fy", type=float, default=None, help="Known focal length fy (pixels); defaults to fx")
    parser.add_argument("--cx", type=float, default=None, help="Known principal point cx (pixels)")
    parser.add_argument("--cy", type=float, default=None, help="Known principal point cy (pixels)")
...
# added build intrinsics / TK
def _build_intrinsics(B, device, W, H, fx=None, fy=None, cx=None, cy=None):
    K = torch.eye(3, device=device).unsqueeze(0).repeat(B, 1, 1)
    if fx is not None:
        K[:, 0, 0] = fx
        K[:, 1, 1] = fy if fy is not None else fx
...
# added firstframe estimation / TK
def _broadcast_firstframe_intrinsics(pts3ds_self, W, H):
    from src.dust3r.post_process import estimate_focal_knowing_depth
    device = pts3ds_self.device
    B = pts3ds_self.shape[0]
    pp0 = torch.tensor([W // 2, H // 2], device=device).float().unsqueeze(0)  # (1,2)
    f0 = estimate_focal_knowing_depth(pts3ds_self[0:1], pp0, focal_mode="weiszfeld")  # (1,)
    fx = f0.repeat(B)
    fy = fx.clone()
    K = torch.eye(3, device=device).unsqueeze(0).repeat(B, 1, 1)
    K[:, 0, 0] = fx
...
def prepare_output(
        ...
        save=False, render=False, render_video=False, img_res=None, subsample=1, args=None):
    """
...
#added extrinsics override (for camera mode static and stationary / TK
    if args is not None:
        if args.camera_mode == "static":
            # fix rotation & translation to the first frame
            R_c2w = R_c2w[0:1].repeat(R_c2w.shape[0], 1, 1)
            t_c2w = t_c2w[0:1].repeat(t_c2w.shape[0], 1)
        elif args.camera_mode == "stationary":
            # zero translation, keep rotation per-frame
            t_c2w = torch.zeros_like(t_c2w)

    # Rebuild 4x4 poses from (possibly overridden) R/t so downstream uses are consistent
    pr_poses = []
    for R, t in zip(R_c2w, t_c2w):
        M = torch.eye(4, device=R.device)
        M[:3, :3] = R
        M[:3, 3]  = t
        pr_poses.append(M.unsqueeze(0))
...
#added intrinsics override / TK
    if args is not None and (args.fx is not None or args.fy is not None or args.cx is not None or args.cy is not None):
        # 1) If user supplied intrinsics, force a constant K across all frames (any mode)
        fx_t = torch.tensor(args.fx, device=pts3ds_self.device).float() if args.fx is not None else None
        fy_t = torch.tensor(args.fy, device=pts3ds_self.device).float() if args.fy is not None else None
        cx_t = torch.tensor(args.cx, device=pts3ds_self.device).float() if args.cx is not None else None
        cy_t = torch.tensor(args.cy, device=pts3ds_self.device).float() if args.cy is not None else None
        intrinsics_tosave = _build_intrinsics(B, pts3ds_self.device, W, H, fx=fx_t, fy=fy_t, cx=cx_t, cy=cy_t)
        # viewer expects a single 'focal' scalar; pass fx
        pp    = torch.stack([intrinsics_tosave[:,0,2], intrinsics_tosave[:,1,2]], dim=-1)
        focal = intrinsics_tosave[:,0,0]

    elif args is not None:
        if args.camera_mode in ["static", "following"]:
            # 2) Otherwise, for static/following, freeze intrinsics from FIRST frame
            intrinsics_tosave = _broadcast_firstframe_intrinsics(pts3ds_self, W, H)
            pp    = torch.stack([intrinsics_tosave[:,0,2], intrinsics_tosave[:,1,2]], dim=-1)
            focal = intrinsics_tosave[:,0,0]
        elif args.camera_mode in ["stationary", "default"]:
            # 3) Keep original per-frame estimates (no changes)
            pass
    # else: no args -> keep original behavior
...
def run_inference(args):
    ...
    (
        ...
        args.render, args.render_video, img_res, args.subsample, args=args
    )
